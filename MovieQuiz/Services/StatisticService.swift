...
final class StatisticServiceImplementation: StatisticServiceProtocol {
    ...
    private(set) var bestGame: GameRecord {
        get {
            guard let data = userDefaults.data(forKey: Keys.bestGame.rawValue), let record = try? JSONDecoder().decode(GameRecord.self, from: data)
            else {
    ...
    func store(correct count: Int, total amount: Int) {
        ...
        guard let data = try? JSONEncoder().encode(currentGame)
        else {
